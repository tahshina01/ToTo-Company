...
const AddLandMarkDialog = ({ dialogRef, isEdit, landmark }) => {
  ...
  const landMarkTypes = ["Cultural", "Historical", "Natural", "Modern"];
    ...
    try {
      ...
      if (!isEdit) {
        response = await axios.post(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/landmark/add`,
          formData,
          {
            headers: {
        ...
        response = await axios.post(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/landmark/edit`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (response.status == 200) {
        setWarning("");
        if (!isEdit) {
          setToastMessage("Landmark added successfully");
        } else {
          setToastMessage("Landmark updated successfully");
        }
          ...
          onClick={() => {
            setTimeout(() => {
              if (!isEdit) {
                ...
                setPrevAttachments(landmark.images);
                setNewAttachments([]);
                setPreviewFiles([]);
                setName(landmark.name);
                setLocation(landmark.location);
                setCategory(landmark.type);
...
export default AddLandMarkDialog;
